...
const DataSourceManager: React.FC = () => {
  ...
  const uploadVcfMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['contacts'] });
      alert('✅ VCF file uploaded successfully!');
    },
    onError: (error: any) => {
      alert(`❌ Failed to upload VCF file: ${error.message}`);
    },
  });
  ...
  const processVcfMutation = useMutation({
    ...
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: ['vcf-files'] });
      queryClient.invalidateQueries({ queryKey: ['contacts'] });
      alert(`✅ VCF file processed successfully! ${response.data?.contactCount || 0} contacts extracted.`);
    },
    onError: (error: any) => {
      alert(`❌ Failed to process VCF file: ${error.message}`);
    },
  });

  const deleteVcfMutation = useMutation({
    mutationFn: (fileId: string) => dataSourceAPI.deleteVcfFile(fileId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['vcf-files'] });
      alert('✅ VCF file deleted successfully!');
    },
    onError: (error: any) => {
      alert(`❌ Failed to delete VCF file: ${error.message}`);
    },
  });
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && (file.name.endsWith('.vcf') || file.name.endsWith('.vcard'))) {
      ...
      alert('Please select a .vcf or .vcard file');
    }
  };
          ...
          {vcfFiles?.data?.map((file: VcfFile) => (
            <div key={file.id} className="border rounded-lg p-4 bg-white shadow-sm">
              ...
              <div className="flex gap-2">
                ...
                <button 
                  onClick={() => deleteVcfMutation.mutate(file.id)}
                  disabled={deleteVcfMutation.isPending}
                  className="px-3 py-2 border rounded text-sm hover:bg-gray-50 text-red-600 disabled:opacity-50"
                >
                  <Trash2 size={14} />
                </button>
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">Database Already Set</h3>
          <p className="text-gray-600">Your invoice database is configured and ready to use.</p>
        </div>
      </div>
    );
  };
